        ...
        {menus.map((menu, index) => (
          <div key={index} className='relative'>
            ...
            {selectedMenu === menu.title && menu.subItems.length > 0 && (
              <ul className={`pl-[75px] pl- mt-2 duration-200 transition-all ease-in-out ${!open ? 'pl-[2px]' : null}`}>
                {menu.subItems.map((subItem, idx) => (
                  <NavLink
                    key={idx}
                    to={subItem.path}
                    className={ ({isActive}) => `flex items-center cursor-pointer text-black font-medium hover:text-gray-400 mb-2 gap-2 stroke-2 ${isActive? 'underline decoration-black decoration-2 hover:text-black' : null} `}
